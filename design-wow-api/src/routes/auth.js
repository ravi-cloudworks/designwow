...
auth.get('/google/callback', async (c) => {
    ...
    if (!user) {
        ...
        await c.env.DB.prepare(`INSERT INTO users (id, email, name, avatar_url, role, google_id) VALUES (?, ?, ?, ?, 'customer', ?)`).bind(id, payload.email, payload.name, payload.picture ?? null, payload.sub).run();
...
auth.get('/me', async (c) => {
    ...
    const user = await c.env.DB.prepare('SELECT id, email, name, avatar_url, role FROM users WHERE id = ?').bind(sessionUserId).first();
