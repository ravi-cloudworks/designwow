...
subscriptions.get('/me', async (c) => {
  ...
  const subscription = await c.env.DB.prepare(
    `SELECT * FROM subscriptions WHERE customer_id = ? AND status != 'cancelled' ORDER BY started_at DESC LIMIT 1`
  ).bind(userId).first();
...
subscriptions.patch('/me', async (c) => {
  ...
  const userId = currentUserId(c);
  if (!userId) return c.json({ error: 'unauthenticated' }, 401);
  const { planTier } = await c.req.json<{ planTier: 'standard' | 'priority' }>();

  const slaHours = planTier === 'priority' ? 48 : 78;
  const amountPaise = planTier === 'priority' ? 699900 : 299900;

  await c.env.DB.prepare(
    `UPDATE subscriptions SET plan_tier = ?, sla_hours = ?, amount_paise = ?
     WHERE customer_id = ? AND status != 'cancelled'`
  ).bind(planTier, slaHours, amountPaise, userId).run();
